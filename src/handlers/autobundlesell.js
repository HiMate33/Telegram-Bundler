...
module.exports = {
  handleAutoBundleSell: async (bot, callbackQuery) => {
    ...
    await bot.sendMessage(chatId, "🔍 Please enter the *token address* you want to bundle-Sell:", {
      parse_mode: "Markdown",
    });
  },
    ...
    if (state.step === "awaiting_token_address") {
      ...
      await bot.sendMessage(
        ...
        `Press *Confirm* to execute.`,
        {
          parse_mode: "Markdown",
          reply_markup: {
            //inline_keyboard: [[{ text: "✅ Confirm Buy", callback_data: "bundle_sell_confirm" }]],
            inline_keyboard: [[{ text: "✅ Confirm Buy", callback_data: "bundle_sell_confirm" }]],
          },
        }
      );
      ...
      return bot.sendMessage(chatId, "💸 Now enter the *amount* (in SOL) each wallet should Sell:");
    }


    else if (state.step === "awaiting_sell_time") {
      ...
      tempInputMap[telegramId].step = "awaiting_amount";
      return bot.sendMessage(chatId, "💸 Enter the *amount (in SOL)* each bundled wallet should Sell:");
    }
  },
  ...
  handleAutoBundleActions: async (bot, callbackQuery) => {
    ...
    if (action === "bundle_sell_now") {
      ...
      return bot.sendMessage(chatId, "💸 Enter the *amount of tokens* each bundled wallet should Sell:");
    }
      ...
      for (let wallet of user.bundled_wallets) {
        try {
          ...
          if (result.success) {
            await bot.sendMessage(
              chatId,
              `✅ [${wallet.publicKey}] Sell successful!\n[View on Solscan](https://solscan.io/tx/${result.signature})`,
              { parse_mode: "Markdown", disable_web_page_preview: false }
            );
          } else {
            await bot.sendMessage(
