      ...
      if (buyType === "Buy Later" && buyTime) {
        ...
        cron.schedule(cronTime, async () => {
          for (let wallet of user.bundled_wallets) {
            let success = Math.random() > 0.2; // Mock: 80% chance of success
            if (success) {
              await bot.sendMessage(chatId, `✅ [${wallet.publicKey}] Buy successful for ${amount} SOL of ${token.symbol}`);
            } else {
              await bot.sendMessage(chatId, `❌ [${wallet.publicKey}] Buy failed for ${amount} SOL of ${token.symbol}`);
            }
          }
